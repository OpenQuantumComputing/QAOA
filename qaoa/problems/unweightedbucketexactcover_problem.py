...
class UnweightedBucketExactCover(Problem):
    """
    Unweighted Bucket Exact Cover (HUBO) problem.
    ...
    In contrast to the BucketExactCover class, this class searches for feasible
    solutions only, and therefore ignoring the weights. 
    ...
    Attributes:
        ...
        weights (np.ndarray): Weights per columns, to enable ranking between feasible solutions in postproc. 
    ...
    def __init__(
        ...
        denom = n_orders * (num_buckets - 1) ** 2
        self.scaling_factor = 1
        if denom != 0:
            self.scaling_factor = self.upper_bound_scaling / denom
    ...
    def compute_cost_for_feasible(self, x, decode=True):
        """
        Calculates the actual weighted cost, but only for feasible solutions
        """
        assert(self.isFeasible(x, decode=decode))
        if decode:
            x = self._decode(x)
    ...
    def cost(self, string):
        """
        Calculates the unweighted exact cover cost (penalty term only) of a given solution.
        ...
        coverage = Y_orders @ x
        return - self.scaling_factor * np.sum((coverage - 1) ** 2)
    
    def isFeasible(self, x, decode=True):
        """
    ...
    def create_circuit(self):
        ...
        if n_orders > 0:
            for o in range(n_orders):
                # Constant term: +1
                add_to_poly(frozenset(), self.scaling_factor)

                # -sum_k g_{o,k}
                for k in range(self.num_buckets):
                    for T, coeff in cov_polys.get((o, k), {}).items():
                        add_to_poly(T, -self.scaling_factor * coeff)
                    ...
                    for k2 in range(k + 1, self.num_buckets):
                        for T1, c1 in cov_polys.get((o, k), {}).items():
                            for T2, c2 in cov_polys.get((o, k2), {}).items():
                                ...
                                add_to_poly(T, 2.0 * self.scaling_factor * c1 * c2)
    ...
    def brute_force_solve(self, return_num_feasible=False):
        ...
        for combo in itertools.product(*bucket_ranges):
            ...
            if self.isFeasible(bitstring):
                num_feasible += 1
                val = self.compute_cost_for_feasible(bitstring)
                if val > best_val:
                    best_val = val
                    best_sol = bitstring
    ...
    def get_hist_cost_vs_ratio(self, encoded_hist):
        """
        Make a histogram only from the feasible solutions, but where
        the key is the weighted cost value. Value is hit ratio
        """
        hist = self.decode_histogram(encoded_hist)
        feasible_hits = {}
        total_shots = sum(hist.values())

        if total_shots == 0:
            return feasible_hits

        for (bitstring, num_hits) in hist.items():
            x = np.array([float(bitstring[j]) for j in self._valid_columns])
            if self.isFeasible(x, decode=False):
                c = self.compute_cost_for_feasible(x, decode=False)
                feasible_hits[c] = feasible_hits.get(c, 0.0) + num_hits/total_shots
        return feasible_hits