...
def _plot_landscape(A, extent, fig=None, title=None):
    ...
    ax.set_xlabel(r"$H_{\text{problem}}(\gamma)$")
    ax.set_ylabel(r"$H_{\text{mixer}}(\beta)$")
...
def plot_angles(qaoa_instance, depth, label, style="", fig=None):
    ...
    ax.plot(
        ...
        angles[::2],
        "--" + style,
        label=r"$H_{\text{problem}}(\gamma)$ " + label,
    )
    ax.plot(
        np.arange(1, depth + 1),
        angles[1::2],
        "-" + style,
        label=r"$H_{\text{mixer}}(\beta)$ " + label,
    )
...
def plot_AllOptimalParameters(qaoa, figsize=(14, 6), title=None):
    ...
    axs[0].set_ylabel(r"$H_{\text{problem}}(\gamma_j)$")
    axs[0].grid()
    ...
    axs[1].grid()
    axs[1].set_ylabel(r"$H_{\text{mixer}}(\beta_j)$")
    ...
    axs[2].set_ylabel(r"$H_{\text{problem}}(\gamma_j)$")
    axs[2].grid()
    ...
    axs[3].grid()
    axs[3].set_ylabel(r"$H_{\text{mixer}}(\beta_j)$")
